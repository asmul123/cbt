    ...
    <section class="section">
        <div class="card">
            <div class="card-header">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 col-12">
                                                <div class="form-group">
                                                    <label for="first-name-column">Nama Ruangan</label>
                ...
                <a href="#" class="btn icon icon-left btn-success float-right" data-toggle="modal" data-target="#import-rombel"><i data-feather="upload"></i> Import Ruangan</a>
                            ...
                            <div class="modal-body">
                                    <div class="row">
                                        <div class="col-md-12 col-12">
                                            ...
                                            <a href="{{ url('/') }}/ruangan/create">Download Format</a>
                                        </div>
                                    </diV>
                                </div>
                                <div class="modal-footer">
                  ...
                  <table class="table table-striped mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $ruangan->tahunpelajaran->tahunpelajaran }}</td>
                            <td>{{ $ruangan->ruangan }}</td>                            
                            @php
                                $total = 0;
                                $kelompoks = App\Models\Kelompok::where('ruangan_id', $ruangan->id)->get();
                                foreach ($kelompoks as $kelompok) {
                                    $jmlanggota = App\Models\Anggotakelompok::where('kelompok_id', $kelompok->id)->count();
                                    $total = $total + $jmlanggota;
                                }
                            @endphp
                            <td>{{  $total }}</td>
                            <td>
                                <div class="btn-group mb-3" role="group" aria-label="Basic example">
                                    <a href="{{ url('/') }}/ruangan/{{ $ruangan->id }}" class="badge icon bg-success"><i data-feather="list"></i></a>
                                    <a href="{{ url('/') }}/ruangan/{{ $ruangan->id }}/edit" class="badge icon bg-warning"><i data-feather="edit"></i></a>
                                    <form action="{{ url('/') }}/ruangan/{{ $ruangan->id }}" method="post">
                                        @method('delete')
                                        @csrf
